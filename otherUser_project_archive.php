<?php 
    ...
    $query_projectUser="select * from User_Accounts where username='$selected_profile'";
    $data_projectUser=mysqli_query($conn,$query_projectUser);
    $projectUser_id_array=mysqli_fetch_array($data_projectUser);
    $projectUser_id=$projectUser_id_array[0];

    $query_project="select * from Projects where user_id='$projectUser_id'";
...
<head>
    ...
    <meta charset="utf-8">
    <link rel="stylesheet" type="text/css" href="../../style/otherUser_project_archive.css">
</head>
...
<div class="top">
    ...
    <p id="pa_title">프로젝트 보관함</p>
        <div class="projectArchive">    
            <?php while($row_project=mysqli_fetch_array($data_project)){?>
                <form action="project_click.php" method="POST">
                <div class="project">
                    <input type="submit" name="selected_proj" value="<?php echo $row_project[0] ?>" class="project_img" style="color:transparent;border:0;outline:0;background-color:transparent;background-image:url('<?php echo $row_project[2] ?>');background-repeat:no-repeat; background-size:370px 370px;" formaction="project_click.php"/>
